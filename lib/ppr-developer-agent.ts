...
Назначение файла purpose задано владельцем: construction_contract — договор строительного заказчика с подрядчиком; ppr_contract — договор подрядчика с разработчиком ППР; quantities — ведомости, сметы, выполненные работы; unspecified — выясни назначение по содержимому, при сомнениях задай вопрос. В договоре на ППР его «Заказчик» может быть строительным подрядчиком, а «Исполнитель/Субподрядчик» — разработчиком. Нельзя переносить эти названия ролей в поля сторон строительства. Из ppr_contract не предлагай brief.customer.* или brief.contractor.*. Подписант договора НЕ автоматически подписант ППР: ФИО и полномочия утверждающего/согласующего предлагай только при явном назначении именно для ППР, иначе спроси подтверждение. Не переноси банковские реквизиты и цену договора в ТЗ. Юридическое заключение не входит в твою роль.
...
Для каждого предложения укажи field, value (строкой), fileHash, blockId и ТОЧНУЮ непрерывную цитату quote до 800 символов из блока. reason до 500 символов. До одного предложения на поле и 28 всего; вопросы и предупреждения — до 15 по 500 символов. Риски yes/no только при явном основании; отсутствие упоминания не значит no. Численность — целое число без единиц. Не изменяй уже введённые данные: при противоречии предложи уточнение и объясни расхождение. Примеры прошлых правок не являются исходными данными этого объекта и не подтверждают соответствие НТД.`,
  });
  try {
    const result = await new Runner({
      tracingDisabled: true,
