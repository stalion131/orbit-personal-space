...
import { ArrowUpRight, ArrowUp, Bot, Check, CheckCheck, Command, Download, GraduationCap, LayoutDashboard, ListTodo, LoaderCircle, Mail, Plus, RefreshCw, Search, Settings2, ShieldCheck, Sparkles, Workflow, X, Plane, Briefcase, UserRound, Clock3 } from 'lucide-react';
...
const sphereIcons = { work: Briefcase, personal: UserRound, travel: Plane, fitness: Workflow, learning: GraduationCap, shopping: ListTodo };
const specialist = { work: 'Рабочий ассистент', personal: 'Личный ассистент', travel: 'Планировщик', fitness: 'Тренер', learning: 'Исследователь', shopping: 'Ассистент покупок' };
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        let result = await api<{tasks: Task[]}>(`/api/tasks?demo=${demo ? 1 : 0}`, {signal: controller.signal});
        if (demo && !result.tasks.length) {
          await api('/api/demo', {method:'POST', body:'{}', signal:controller.signal});
          result = await api<{tasks: Task[]}>('/api/tasks?demo=1', {signal:controller.signal});
        }
  ...
  function switchMode(value: boolean) { if(value === demo) return; setTasks([]); setSelected(null); setDemo(value); setNotice(''); setLoading(true); navigate('overview'); }
  function newTask() { switchMode(false); navigate('overview'); setTimeout(()=>draftRef.current?.focus(), 0); }
  ...
  async function submit(event: React.SyntheticEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setTasks(demo ? [task] : current=>[task,...current.filter(item=>item.id!==task.id)]);
      setDemo(false); navigate('overview'); setDraft(''); requestId.current=null; setRefresh(value=>value+1);
  ...
  async function act(task: Task, operation: Operation) {
    ...
    try {
      ...
      setNotice(result.task.result || 'Демонстрационный шаг завершён. Предложение готово к просмотру.');
    } catch(err) {setError(err instanceof Error ? err.message : 'Не удалось записать решение.');}
    finally {setBusy(false);}
  }
  function exportTasks() {
    const blob = new Blob([JSON.stringify({format:'orbit-tasks-v1', exportedAt:new Date().toISOString(), demo, tasks},null,2)],{type:'application/json'});
    const url=URL.createObjectURL(blob); const link=document.createElement('a'); link.href=url; link.download=`orbit-${demo?'demo':'tasks'}.json`; link.click(); setTimeout(()=>URL.revokeObjectURL(url),1000);
  }
  const stats = [
    {icon:ListTodo,label:'Всего задач',value:tasks.length,caption:demo?'В демонстрационном наборе':'Во всех сферах жизни',filter:'all'},
    {icon:Workflow,label:'В работе',value:tasks.filter(t=>t.status==='running').length,caption:demo?'Демонстрационный статус':'Выполняются агентами',filter:'running'},
    {icon:ShieldCheck,label:'Ждут решения',value:approvals.length,caption:'Нужно ваше внимание',filter:'approval'},
    {icon:CheckCheck,label:'Завершено',value:tasks.filter(t=>t.status==='completed').length,caption:demo?'Демонстрационный результат':'Завершённые задачи',filter:'completed'},
  ];
  ...
  return <div className="workspace">
    ...
    <div className="main-shell"><header className="topbar"><div>Рабочее пространство <span>/</span> <strong>{view==='overview'?'Обзор':view==='tasks'?'Все задачи':'Подтверждения'}</strong></div><span className="local-status"><i/>Локальный режим</span></header>
      <main className="main-content">
        ...
        <div className="mode-row"><div className="mode-switch" aria-label="Источник задач"><Button variant={demo?'secondary':'ghost'} aria-pressed={demo} onClick={()=>switchMode(true)} disabled={busy}>Демо-примеры</Button><Button variant={!demo?'secondary':'ghost'} aria-pressed={!demo} onClick={()=>switchMode(false)} disabled={busy}>Мои задачи</Button></div><Button variant="ghost" className="export-button" disabled={loading || !tasks.length} onClick={exportTasks}><Download size={14}/>Экспорт</Button></div>
        <div className={`demo-banner ${demo?'':'real-banner'}`}><Sparkles size={15}/><span>{demo?'Это примеры, а не результаты работы ИИ. Можно безопасно проверить подтверждения.':'Ваши задачи сохраняются локально. ИИ-агенты и внешние действия пока отключены.'}</span><span className="demo-label">{demo?'ДЕМО':'MVP'}</span></div>
        ...
        <div className="content-grid"><section className="task-section" aria-label="Список задач">
          <div className="section-heading"><h2>{view==='approvals'?'Ожидают решения':demo?'Примеры задач':'Мои задачи'} <span>{filtered.length}</span></h2><Button variant="ghost" onClick={()=>setRefresh(v=>v+1)} disabled={loading||busy} aria-label="Обновить задачи"><RefreshCw size={14} className={loading?'spin':''}/></Button></div>
        ...
        <div className="right-column"><section className="approval-panel"><div className="section-heading"><h2><ShieldCheck size={18}/>На подтверждение</h2><span className="count">{approvals.length}</span></div>
          {approvals.length?approvals.map(task=><div className="approval-preview" key={task.id}><div className="approval-kicker">{spheres.find(item=>item.id===task.sphere)?.name.toUpperCase()} <span>{task.demo?'ПРИМЕР':''}</span></div><h3>{task.proposal?.title}</h3><p>{task.sphere==='travel'?'Условный вариант проживания. Доступность и стоимость не проверялись.':'Черновик готов к просмотру. Ничего не будет отправлено.'}</p>{task.sphere==='travel'&&<div className="proposal-price"><span>2 ночи · пример</span><strong>240 €</strong></div>}<Button className="review-button" onClick={()=>setSelected(task.id)}>Посмотреть предложение<ArrowUpRight size={15}/></Button></div>):<div className="clear-approvals"><ShieldCheck size={27}/><h3>Всё спокойно</h3><p>Нет предложений, ожидающих решения.</p></div>}
    ...
    <Dialog open={!!selectedTask} onOpenChange={open=>{if(!open&&!busy)setSelected(null);}}><DialogContent className="task-dialog"><DialogHeader><div className="detail-kicker">{spheres.find(item=>item.id===selectedTask?.sphere)?.name}{selectedTask?.subcategory?` / ${selectedTask.subcategory}`:''} {selectedTask?.demo?' / ДЕМОНСТРАЦИЯ':' / МОЯ ЗАДАЧА'}</div><DialogTitle className="detail-title">{selectedTask?.title}</DialogTitle><DialogDescription>Задача, предложение и полная история действий.</DialogDescription></DialogHeader>
      ...
      <div className="detail-actions">{selectedTask.demo&&!selectedTask.proposal&&['pending','running','error'].includes(selectedTask.status)&&<Button className="primary-button" disabled={busy||loading} onClick={()=>void act(selectedTask,{op:'simulate'})}><Sparkles size={15}/>{selectedTask.status==='error'?'Повторить демо-шаг':'Продолжить демо'}</Button>}{selectedTask.status==='pending'&&<Button variant="outline" disabled={busy||loading} onClick={()=>void act(selectedTask,{op:'complete'})}><CheckCheck size={15}/>Завершить вручную</Button>}</div></>}
    </DialogContent></Dialog>
    <Dialog open={!!info} onOpenChange={open=>{if(!open)setInfo(null);}}><DialogContent className="info-dialog"><DialogHeader><DialogTitle>{info==='agents'?'Будущая команда агентов':'Orbit · локальный MVP'}</DialogTitle><DialogDescription>{info==='agents'?'План подключения через OpenAI Agents SDK. Эти агенты пока не работают.':'Что уже работает и где проходят границы прототипа.'}</DialogDescription></DialogHeader>{info==='agents'?<><div className="orchestrator"><Workflow size={21}/><div><strong>Главный оркестратор</strong><p>Принимает задачу, планирует шаги и ждёт ваших решений.</p></div></div><div className="agents-grid">{[{name:'Классификатор',description:'Определяет категорию и план',icon:ListTodo},{name:'Исследователь',description:'Ищет и сравнивает информацию',icon:Search},{name:'Планировщик',description:'Готовит маршруты и варианты',icon:Plane},{name:'Почтовый агент',description:'Подготавливает черновики писем',icon:Mail}].map(agent=><div key={agent.name}><agent.icon size={19}/><h3>{agent.name}</h3><p>{agent.description}</p><small>Не подключён</small></div>)}</div><p className="info-note">Следующий этап: Python-сервис с цепочкой «классификатор → исследователь → отчёт». Реальные действия подключаются только после серверной проверки подтверждения.</p></>:<div className="about-copy"><p><strong>Работает:</strong> создание задач, поиск, фильтры, ручное завершение, демо-предложения, история решений и экспорт.</p><p><strong>Хранение:</strong> локальная SQLite-база через D1-эмулятор. Перезагрузка страницы не удаляет данные. Демо и ваши задачи разделены.</p><p><strong>Отключено:</strong> вызовы LLM, поиск в интернете, почта, платежи и бронирования. API-ключи не требуются.</p><p><strong>Безопасность:</strong> только этот компьютер. Авторизация для облака ещё не реализована — не публикуйте текущую версию и не открывайте доступ по сети.</p><p>Кнопка «Экспорт» сохраняет задачи текущего режима в JSON. Автоматический импорт пока не предусмотрен.</p></div>}</DialogContent></Dialog>
  </div>;
}
