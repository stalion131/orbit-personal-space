...
export async function POST(request: Request) {
  try {
    ...
    if (data.question !== undefined && (typeof data.question !== 'string' || data.question.length > 2000)) throw new TaskError('Вопрос должен быть не длиннее 2000 символов.');
    if (data.dialogue !== undefined && (!Array.isArray(data.dialogue) || data.dialogue.length > 4 || data.dialogue.some(item => !item || typeof item !== 'object' || typeof item.question !== 'string' || item.question.length > 2000 || typeof item.answer !== 'string' || item.answer.length > 1000))) throw new TaskError('Некорректная история диалога.');
    ...
    if (data.revision !== undefined && data.revision !== task.revision) throw new TaskError('Проект изменился. Подтвердите запрос по актуальному ТЗ.', 409);
    ...
    return json({ result: await analyzePprProject(task, task.workProject, data.question as string | undefined, data.dialogue as { question: string; answer: string }[] | undefined, request.signal) });
