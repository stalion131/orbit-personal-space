...
export async function GET(request: Request) {
  try { guard(request); return json({tasks: await listTasks(new URL(request.url).searchParams.get('demo') === '1')}); } catch (error) { return failure(error); }
}
export async function POST(request: Request) {
  try {
    guard(request); const data = await body(request);
    ...
    const task = await insertTask(createTask(data.id, data.description.trim(), data.sphere as SphereId, data.subcategory, data.dueDate as string|null, Number(data.queue), data.priority as Priority));
